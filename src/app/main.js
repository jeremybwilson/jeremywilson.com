...
theme.Header = (function() {
  function Header(container) {
    ...
    const ui = {
      ...
      promoSlide: $( '#double-promo-wrapper .promo-banner' ),
      promoTrigger: $( '.promo-banner > a' ),
      multiPromo: $container.attr('data-button')
    }
    const self = this;

    // PROMO BANNER : If 2+ Promos enabled, connect swapping and arrow functionality
    // if ( ui.promoWrap && ui.arrows && ui.swapRate ) {
    if ( ui.promoWrap && ui.swapRate ) {

      const swapPromos = (banner_type) => {
        if(banner_type == 'multi'){
          // if the multiPromo attribute is found, write in the class name
          if(ui.promoWrap.find('.promo-banner.active').next().filter('.promo-banner').length == 0){
            ui.promoWrap.find('.promo-banner').removeClass('active');
            ui.promoWrap.find('.promo-banner:first').addClass('active');
          } else {
            ui.promoWrap.find('.promo-banner.active').next().filter('.promo-banner').addClass('active');
            ui.promoWrap.find('.promo-banner.active:first').removeClass('active');
          };
        }
      }
